...
let bonuses = [], activeBonus = null, bonusUntil = 0;
const bonusTypes = [{type:'speed',label:'PLUS VITE',color:'#61d9df',chance:.16},{type:'wide',label:'TIR LARGE',color:'#ffd166',chance:.13},{type:'double',label:'DOUBLE TIR',color:'#ff8fab',chance:.09},{type:'triple',label:'TRIPLE TIR',color:'#c9a7ff',chance:.04},{type:'life',label:'+1 VIE',color:'#ff5d7b',chance:.025}];
...
function spawnEnemy(){const d=dimensions(),t=enemyTypes[Math.floor(Math.random()*enemyTypes.length)];enemies.push({...t,x:35+Math.random()*(d.w-70),y:-30,phase:Math.random()*6});}
function shoot(){const now=performance.now();if(now-lastShot<280)return;const type=activeBonus&&bonusUntil>now?activeBonus.type:null;const offsets=type==='triple'?[-18,0,18]:type==='double'?[-11,11]:[0];offsets.forEach(offset=>shots.push({x:player.x+offset,y:player.y-42,wide:type==='wide'}));lastShot=now;beep(520,.04);}
...
function collectBonus(b){bonuses=bonuses.filter(x=>x!==b);if(b.type==='life')lives=Math.min(5,lives+1);else{activeBonus=b;bonusUntil=performance.now()+10000;}updateHud();}
...
function update(dt,time){
  const d=dimensions(),now=performance.now();if(activeBonus&&bonusUntil<=now)activeBonus=null;const speed=activeBonus&&activeBonus.type==='speed'?440:270;if(keys.ArrowLeft||keys.a)player.x-=speed*dt;if(keys.ArrowRight||keys.d)player.x+=speed*dt;player.x=Math.max(34,Math.min(d.w-34,player.x));if(keys[' '])shoot();
  if(time-lastSpawn>920){spawnEnemy();lastSpawn=time;} shots.forEach(s=>s.y-=470*dt);enemies.forEach(e=>{e.y+=e.speed*dt;e.phase+=dt*3});bonuses.forEach(b=>{b.y+=b.vy*dt;if(Math.hypot(b.x-player.x,b.y-player.y)<38)collectBonus(b)});bonuses=bonuses.filter(b=>b.y<d.h+30);
