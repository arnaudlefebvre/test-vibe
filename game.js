...
function shoot(){const now=performance.now();if(now-lastShot<280)return;shots.push({x:player.x,y:player.y-42});lastShot=now;beep(520,.04);}
...
function update(dt,time){
  const d=dimensions(); if(keys.ArrowLeft||keys.a)player.x-=270*dt;if(keys.ArrowRight||keys.d)player.x+=270*dt;player.x=Math.max(34,Math.min(d.w-34,player.x));if(keys[' '])shoot();
  if(time-lastSpawn>920){spawnEnemy();lastSpawn=time;} shots.forEach(s=>s.y-=470*dt);enemies.forEach(e=>{e.y+=e.speed*dt;e.phase+=dt*3});
  for(let i=enemies.length-1;i>=0;i--){const e=enemies[i];for(let j=shots.length-1;j>=0;j--){const s=shots[j];if(Math.hypot(e.x-s.x,e.y-s.y)<29){score+=e.points;destroyed++;for(let p=0;p<10;p++)particles.push({x:e.x,y:e.y,vx:(Math.random()-.5)*130,vy:(Math.random()-.5)*130,life:1,color:e.color});enemies.splice(i,1);shots.splice(j,1);beep(760,.08);updateHud();break;}}if(enemies[i]&&enemies[i].y>d.h-35){enemies.splice(i,1);lives--;updateHud();beep(150,.15);if(lives<=0)endGame();}}
