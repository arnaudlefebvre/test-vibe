...
function spawnEnemy(){const d=dimensions(),t=enemyTypes[Math.floor(Math.random()*enemyTypes.length)];const levelFactor=1+(level-1)*GAME_CONFIG.enemySpeedPerLevel+(wave-1)*.05;const hitPoints=1+(level-1)+Math.floor((wave-1)/2);enemies.push({...t,hp:hitPoints,speed:t.speed*levelFactor*(.8+Math.random()*.4),x:35+Math.random()*(d.w-70),y:-30,phase:Math.random()*6});}
