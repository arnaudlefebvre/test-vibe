...
function shoot(){const now=performance.now();if(now-lastShot<280)return;const angles=activeBonuses.five?[-60,-30,0,30,60]:activeBonuses.triple?[-60,0,60]:[0];const multiplier=activeBonuses.double?2:1;angles.forEach(angle=>{for(let i=0;i<multiplier;i++){const rad=angle*Math.PI/180;shots.push({x:player.x,y:player.y-42,vx:Math.sin(rad)*470,vy:-Math.cos(rad)*470,wide:!!activeBonuses.wide});}});lastShot=now;beep(520,.04);}
